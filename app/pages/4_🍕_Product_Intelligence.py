...
st.markdown('<p class="section-subtitle">Highest revenue-generating products — color indicates customer rating</p>', unsafe_allow_html=True)

top10 = product_stats.head(10)
fig_top = px.bar(top10, x="total_revenue", y="product", orientation="h",
                 color="avg_rating", color_continuous_scale=["#fca5a5", "#fde68a", "#86efac"],
                 labels={"total_revenue": "Revenue (₹)", "product": "", "avg_rating": "Rating"})
fig_top.update_layout(**PLOTLY_LAYOUT, height=450, coloraxis_colorbar=dict(title="Rating", thickness=12))
fig_top.update_traces(hovertemplate="<b>%{y}</b><br>Revenue: ₹%{x:,.0f}<br>Rating: %{marker.color:.2f}<extra></extra>")
...
st.markdown('<p class="section-subtitle">Lowest revenue products — color indicates cancellation rate (red = high)</p>', unsafe_allow_html=True)
...
fig_bot = px.bar(bottom10, x="total_revenue", y="product", orientation="h",
                 color="cancel_rate", color_continuous_scale=["#86efac", "#fde68a", "#fca5a5"],
                 labels={"total_revenue": "Revenue (₹)", "product": "", "cancel_rate": "Cancel %"})
fig_bot.update_layout(**PLOTLY_LAYOUT, height=400, coloraxis_colorbar=dict(title="Cancel %", thickness=12))
fig_bot.update_traces(hovertemplate="<b>%{y}</b><br>Revenue: ₹%{x:,.0f}<br>Cancel: %{marker.color:.1%}<extra></extra>")
...
with col1:
    ...
    fig_pd.update_traces(hovertemplate="%{y:,}<extra></extra>")
...
st.markdown('<p class="section-subtitle">Hierarchical view of revenue across categories and products</p>', unsafe_allow_html=True)
...
st.markdown('<p class="section-subtitle">Data-driven suggestions to maximize revenue and minimize churn</p>', unsafe_allow_html=True)
...
if len(star_products) > 0:
    st.success("🌟 **Star Products** — High demand + above-average rating. Recommended for promotion and upselling:")
    for _, row in star_products.iterrows():
        st.markdown(f"- **{row['product']}** — {row['total_orders']:,} orders · "
                    f"₹{row['total_revenue']:,.0f} revenue · "
                    f"⭐ {row['avg_rating']:.1f}")
else:
    st.info("No star products identified with current filter selection.")
...
if len(problem_products) > 0:
    st.warning("⚠️ **Needs Attention** — Low demand + below-average rating. Consider recipe improvements or menu repositioning:")
    for _, row in problem_products.iterrows():
        st.markdown(f"- **{row['product']}** — {row['total_orders']:,} orders · "
                    f"⭐ {row['avg_rating']:.1f} · "
                    f"Cancel rate: {row['cancel_rate']*100:.1f}%")
else:
    st.info("No underperforming products detected with current filters.")
...
if len(high_cancel) > 0:
    ...
    st.info("No high-cancellation products detected.")
