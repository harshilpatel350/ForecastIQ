...
has_revenue = "total_amount" in cols
...
if has_revenue:
    ...
    total_revenue = aov = growth = 0

kpi_cols = st.columns(6)
with kpi_cols[0]:
    kpi_card("Total Revenue", f"₹{total_revenue:,.0f}" if has_revenue else "N/A",
             f"{growth:+.1f}% growth" if has_revenue else None, growth > 0, "💰")
with kpi_cols[1]:
    days = filtered['date'].nunique() if "date" in cols else 1
    kpi_card("Total Orders", f"{total_orders:,}", f"{total_orders/max(days,1):.0f}/day avg", True, "📦")
with kpi_cols[2]:
    kpi_card("Avg Order Value", f"₹{aov:,.0f}" if has_revenue else "N/A", None, True, "🧾")
with kpi_cols[3]:
    if "customer_id" in cols:
        kpi_card("Customers", f"{filtered['customer_id'].nunique():,}", None, True, "👥")
    else:
        kpi_card("Columns", f"{len(cols)}", None, True, "📑")
with kpi_cols[4]:
    if "rating" in cols:
        avg_rating = filtered["rating"].mean() if total_orders > 0 else 0
        kpi_card("Avg Rating", f"{avg_rating:.2f} ⭐", None, avg_rating >= 3.5, "⭐")
    else:
        kpi_card("Records", f"{total_orders:,}", None, True, "📦")
with kpi_cols[5]:
...
# REVENUE & ORDERS TABS
# ════════════════════════════════════════════════════════════════════════════
if "date" in cols and has_revenue:
    st.markdown('<div class="section-title">📈 Revenue & Order Trends</div>', unsafe_allow_html=True)
    st.markdown('<p class="section-subtitle">Daily metrics with 7-day moving average overlay</p>', unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════════════════
if ("category" in cols or "city" in cols) and has_revenue:
...
# DEMAND HEATMAP
# ════════════════════════════════════════════════════════════════════════════
if "weekday" in cols and "hour" in cols and "order_id" in cols:
...
# PAYMENT & ORDER TYPE
# ════════════════════════════════════════════════════════════════════════════
if has_revenue and ("payment_method" in cols or "order_type" in cols):
